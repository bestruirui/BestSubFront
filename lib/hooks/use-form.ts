...
export const validators = {
  required: (message = '此字段为必填项') => (value: unknown) =>
  ...
  url: (message = '请输入有效的URL') => (value: string) =>
    value && !/^https?:\/\/.+/.test(value) ? message : undefined,

  minLength: (min: number, message?: string) => (value: string) =>
    value && value.length < min ? (message || `最少需要${min}个字符`) : undefined,

  maxLength: (max: number, message?: string) => (value: string) =>
    value && value.length > max ? (message || `最多允许${max}个字符`) : undefined,
}
