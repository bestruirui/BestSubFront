...
export function useNotifyForm({ onSuccess }: UseNotifyFormProps) {
    ...
    const loadChannelConfig = useCallback(async (channel: string) => {
        try {
            ...
            const configs = await dashboardApi.getNotifyChannelConfig(getToken() || '', channel)
    ...
    const handleSubmit = useCallback(async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (editingNotify) {
                await dashboardApi.updateNotify(editingNotify.id, requestData, getToken() || '')
                toast.success('通知配置更新成功')
            } else {
                await dashboardApi.createNotify(requestData, getToken() || '')
    ...
    }, [formData, editingNotify, getToken, onSuccess, channelConfigs])
